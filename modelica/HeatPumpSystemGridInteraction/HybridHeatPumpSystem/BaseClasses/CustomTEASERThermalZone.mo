...
model CustomTEASERThermalZone
  ...
  Modelica.Blocks.Math.MultiSum multiSum[nZones](each final nu=3)
    if use_verboseEnergyBalance                                                               annotation (Placement(transformation(extent={{-9,-9},
            {9,9}},
        rotation=180,
        origin={69,-129})));

  BESMod.Utilities.Electrical.RealToElecCon
                                       realToElecCon(use_souGen=false)
  ...
  Modelica.Blocks.Math.MultiSum multiSum1(each final nu=2)
    if use_verboseEnergyBalance annotation (Placement(transformation(
        extent={{-9,-9},{9,9}},
        rotation=180,
        origin={157,-79})));
equation
  ...
  connect(realToElecCon.internalElectricalPin, internalElectricalPin)
    annotation (Line(
      points={{83.8,-96.2},{76,-96.2},{76,-96},{70,-96}},
      color={0,0,0},
      thickness=1));
  ...
  connect(realPassThroughTDry.y, thermalZone.ventTemp) annotation (Line(points=
          ...
          {0,0,127}));
  connect(preAbsHeaGaiRad.port, thermalZone[1].intGainsRad) annotation (Line(points={
          {162,110},{170,110},{170,132},{-80,132},{-80,60},{-56,60},{-56,60.24},{-39.74,
          60.24}}, color={191,0,0}));
  connect(preAbsHeaGaiCon.port, thermalZone[1].intGainsConv) annotation (Line(points=
         {{122,112},{134,112},{134,128},{-76,128},{-76,50},{-42,50},{-42,52},{-39.74,
          52},{-39.74,49.44}}, color={191,0,0}));
  connect(preAbsHeaGaiRad.Q_flow, useProBus.absIntGaiRad) annotation (Line(points=
      ...
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(preAbsHeaGaiCon.Q_flow, useProBus.absIntGaiConv) annotation (Line(points=
         {{102,112},{94,112},{94,82},{51,82},{51,101}}, color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(multiSum1.u[1], useProBus.absIntGaiConv) annotation (Line(points={{
          166,-77.425},{160,-77.425},{160,66},{51,66},{51,101}}, color={0,0,127}),
      Text(
      string="%second",
      index=1,
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(multiSum1.u[2], useProBus.absIntGaiRad) annotation (Line(points={{166,
          -80.575},{160,-80.575},{160,50},{51,50},{51,101}}, color={0,0,127}),
      Text(
      string="%second",
      index=1,
      extent={{-3,6},{-3,6}},
      horizontalAlignment=TextAlignment.Right));
  connect(multiSum1.y, realToElecCon.PEleLoa) annotation (Line(points={{146.47,
          -79},{106,-79},{106,-100}}, color={0,0,127}));
